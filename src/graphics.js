	...
	for (i=0;i<clients.length;i++){
		...
		if(movements[username] && player_name!=""){
			...
			if(clients[i].showLastMessage && clients[i]) {
				...
				let txtx=pos_array[username][0]-printx+(centerx+0.5)*TILESIZE-txtlen/2;
				let txty=pos_array[username][1]-printy+(centery-0.25)*TILESIZE;
				txtx=Math.max(txtx,10);
				txty=Math.max(txty,10);
				ctx.drawImage(msg_img,50,0,50,50,txtx,txty,txtlen,40);
				ctx.drawImage(msg_img,0,0,10,50,txtx-10,txty,10,40);
				ctx.drawImage(msg_img,140,0,10,50,txtx+txtlen,txty,10,40);
				ctx.textAlign = "center";
				ctx.fillText(clients[i].lastMessage, txtx+txtlen/2,txty+30);
			};
		}
	}
	sprite_timer++;
